...
data "template_file" "mp_variables_template" {
  ...
  vars {
    ...
    mp_listing_resource_version = "${var.mp_versions[var.fortigate_version]}"
    mp_listing_resource_id      = "${var.mp_listing_resource_id[var.mp_versions[var.fortigate_version]]}"
  }
}
