...
FortiGate NGFWs inspect traffic as it enters and leaves a Virtual Cloud Network. These inspections happen at an unparalleled speed, scale, and performance and prevent everything from ransomware to DDoS attacks, without degrading user experience or creating costly downtime.
...
The following table shows recommended OCI [Compute VM Standard Shapes](https://docs.cloud.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm) configuration. The primary requirement for the provisioning of a virtual FortiGate may be the number of interfaces it can accommodate rather than its processing capabilities.

The licensing for FortiGate-VM does not restrict whether the FortiGate can work on a VM instance that uses more CPUs than the license allows. The number of vCPUs indicated by the license does not restrict the FortiGate from working, regardless of how many CPUs are included in the virtual instance. However, only the licensed number of vCPUs process traffic and management. The rest of the vCPUs are unused.

    > `FortiGate 6.4`
| Model Name       | CPU  | Recommended OCI Compute Shape         |  # of vNICs    |
|------------------|------|---------------------------------------|----------------|
| FG-VM02/02v/02s  |  1   |            VM.Standard2.1             |       2        |
| FG-VM04/04v/04s  |  2   |            VM.Standard2.2             |       2        |
| FG-VM08/08v/08s  |  4   |            VM.Standard2.4             |       4        |
| FG-VM16/16v/16s  |  8   |            VM.Standard2.8             |       8        |
| FG-VM32/32v/32s  |  16  |            VM.Standard2.16            |       16       |
| FG-VMUL/ULv/ULs  | ANY  |            VM.Standard2.1-24          |       2-16     |

Note: A Compute shape also define number of CPUs, maximum number of vNICs and maximum bandwidth. Each OCPU is equivalent of 2 vCPUs (hyperthreads).

## Solutions

### Simple

Simple's deploy a standalone VM of FortiGate NGFW on a Virtual Cloud Network. Simple can be customized to subscribe to different FortiGate images. By default, it subscribes to OCI FortiGate BYOL listing.

![FortiGate NGFW Simple Architecture](./simple/images/fortigate-simple-architecture.png)
